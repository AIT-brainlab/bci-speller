...
def run_visualizer_process(
    ...
    """Run the visualizer process.

    Args:
        data_q: Queue for incoming EEG data chunks.
        marker_q: Queue for incoming marker events.
        stop_event: Event to signal the process to stop.
        pause_event: Event to signal the process to pause.
        n_channels: Number of EEG channels.
        ch_indices: Indices of channels to display.
        window_sec: Window size in seconds.
        fs: Sampling frequency in Hz.
        plot_hz: Animation frame rate in Hz.
        amplitude_uv: Amplitude range in microvolts.
        labels: Channel labels.
        monitor_index: Monitor index for positioning.
    """
    import matplotlib
    ...
    ax_header.text(
        0.99,
        0.5,
        f"Window: {window_sec}s  |  +/-{amplitude_uv} uV",
        transform=ax_header.transAxes,
        fontsize=9,
        va="center",
        ha="right",
    ...
    for ch in range(n_channels):
        ...
        ax_q.set_axis_off()
        ax_q.add_patch(
            plt.Rectangle((0.1, 0.05), 0.8, 0.9, color="#333333", zorder=1)
        )
        (qpatch,) = ax_q.fill(
            [0.1, 0.9, 0.9, 0.1],
            [0.05, 0.05, 0.95, 0.95],
    ...
    try:
        ...
        print(f"[EEGVisualizer] Error: {exc}")
    finally:
        print("[EEGVisualizer] Window closed")


_run_visualizer_process = run_visualizer_process
