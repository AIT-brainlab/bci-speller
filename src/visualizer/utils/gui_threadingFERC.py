...
class WindowBuilder:
    ...
    def __init__(self, size=None, refresh_rate=None, screen=0):
        window = None
        color = (1,1,1)
        if( isinstance(size, type(None)) == False):
            window = visual.Window(size=size, screen=0, units="pix", color=color, blendMode='avg', useFBO=True, fullscr=True)
            window.refreshThreshold = (1/refresh_rate)*1.2
            ...
            window.refreshThreshold = (1/screen.refresh_rate)*1.2
